...
@if ($beritaHangat->isEmpty())
    <section class="py-12 px-6 text-center">
        <p class="text-lg text-gray-500">Tidak ada artikel di section <strong>Berita Hangat</strong>.</p>
    </section>
@else
            ...
            @foreach ($beritaHangat->take(3) as $index => $article)
                <a href="{{ route('articles.show', $article->id) }}" class="block w-full shrink-0 relative group">
                    <img src="{{ asset('storage/' . $article->thumbnail) }}" loading="lazy"
                        ...
                        alt="{{ $article->title }}" />
                    <div
                        class="absolute inset-0 bg-black bg-opacity-25 group-hover:bg-opacity-50 transition duration-500">
                    </div>
