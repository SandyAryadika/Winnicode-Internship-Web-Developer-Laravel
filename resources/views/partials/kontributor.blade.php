...
<section class="px-6 py-12 bg-[#F2F4FF] rounded-lg">
        ...
        <div class="flex gap-12 overflow-x-auto justify-center px-6 py-6">
            @foreach ($topContributors as $contributor)
                <a href="{{ route('authors.show', $contributor->id) }}"
                    ...
                    <img src="{{ $contributor->photo ? asset('storage/' . $contributor->photo) : asset('images/default.png') }}"
                        loading="lazy" class="w-32 h-32 rounded-full mx-auto mb-2 object-cover"
    ...
    @else
        <div class="text-center py-12">
            <p class="text-gray-500 text-lg">Tidak ada Kontributor di section <strong>Kontributor Teratas</strong>.</p>
        </div>
    @endif
</section>
