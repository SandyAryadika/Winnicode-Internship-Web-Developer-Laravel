...
@section('content')
    <section class="px-4 sm:px-6 lg:px-20 py-8 min-h-screen">
        ...
        @if ($matchedAuthors->count())
            <div class="mb-12 border-t">
                <h3 class="text-xl font-semibold mb-6 mt-4 text-black-700 flex items-center gap-2">
                    <img src="{{ asset('images/result-author.png') }}" alt="Author Icon" class="w-8 h-8">
                    Penulis yang cocok:
                </h3>
                        ...
                        <a href="{{ route('authors.show', $author->id) }}"
                            ...
                            <div class="flex flex-col items-center text-center">
                                <img src="{{ $author->photo ? asset('storage/' . $author->photo) : asset('images/default-profile.png') }}"
                                    alt="{{ $author->name }}" class="w-28 h-28 object-cover rounded-full border mb-4">
        ...
        @if ($results->count())
            <h3 class="text-xl font-semibold mb-6 mt-4 text-black-700 flex items-center gap-2">
                <img src="{{ asset('images/result-article.png') }}" alt="Article Icon" class="w-8 h-8">
                Artikel yang sesuai:
            </h3>
